function tests_results() {
    ...
    .then(results => {
        ...
        const container = document.querySelector('.good_tests');
...
function olymps_results() {
    ...
    fetch(`https://olympiad-api.falpin.ru/users/${user.id}/olympiads`, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${jwtToken}`
    ...
    .then(results => {
        ...
        results.forEach(test => {
            ...
            // Добавляем информацию об олимпиаде
            testElement.innerHTML = `
                <h4 class="h4 normal">${test.olympiad_title}</h4>
                <p class="p2 normal">Дата прохождения: ${formattedDate}</p>
                <p class="p2 normal">Результат: ${test.score} из ${test.total_score}</p>
                ${test.grade ? `<p class="p2 normal">Оценка: ${test.grade}</p>` : ''}
                <hr>
